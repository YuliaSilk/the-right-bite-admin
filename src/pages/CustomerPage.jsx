  ...
  return (
    ...
    >
      <Card className="rounded-xl shadow-lg mb-4 text-center">
        ...
        <Title level={3} className="!mt-2 !mb-1">{customer.firstname} {customer.lastname}</Title>
        <Tag color="green" className="mt-3 rounded-full">{customer.orderStatus}</Tag>
      </Card>

      <Card title="Contact Info" className="rounded-xl shadow-lg mb-4">
        <Descriptions column={1} size="small">
          <Descriptions.Item label="Phone"><PhoneOutlined className="mr-2 text-gray-400" />{customer.phoneNumber}</Descriptions.Item>
          {customer.email && (
            <Descriptions.Item label="Email"><MailOutlined className="mr-2 text-gray-400" />{customer.email}</Descriptions.Item>
          )}
      ...
      <Card title="Order Info" className="rounded-xl shadow-lg">
        <Descriptions column={1} size="small">
...
export default function CustomersPage() {
      ...
      try {
        ...
        const mapped = res.map((order) => {
          ...
          return {
            ...
            email: order.user?.email || d.email || "—",            city: d.city,
            country: d.country,
  ...
  const columns = [
    {
        ...
        render: (_, record) => (
          <Space>
            ...
            <div>
              <Text strong className="flex justify-center items-center self-center">
                {record.id} 
              </Text>
      ...
      render: (_, record) => (
        <Dropdown
          overlay={
            <Menu
              items={[
                { key: "view", label: "View Details", onClick: () => setSelectedCustomer(record) },
              ]}
            />
          }
          trigger={["click"]}
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        title={<Title level={4} className="!mb-0 !text-black">Customer List</Title>}
      ...
      <CustomerDetailsDrawer customer={selectedCustomer} onClose={() => setSelectedCustomer(null)} />
    </div>
  );
}